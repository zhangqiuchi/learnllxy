...
{
	return *(int*)a - *(int*)b;//左参数减去又参数 从小到大  右参减去左参 从大到小
	/*
	如果 compar 返回值小于 0（< 0），那么 a 所指向元素会被排在 b 所指向元素的前面
	//如果 compar 返回值等于 0（= 0），那么 p1 所指向元素与 p2 所指向元素的顺序不确定
	如果 compar 返回值大于 0（> 0），那么 a 所指向元素会被排在 b 所指向元素的后面
	*/
}

//========================================================================================================
/*桶排序*/
// 桶排序：分配 和 收集：设计K个桶，然后将n个输入数分布到各个桶中去，对各个桶中的数进行排序，然后
//按照次序把各个桶中的元素排列出来即可 //（美国旧金山大学算法可视化网站） *30/1000
//n ~ nlogn
//========================================================================================================
//========================================================================================================
/*基数排序*/
...
int main(void)
{
	...
	//insertSort( arr, 100000);//插入排序 9.557000 s
	//bubbleSort( arr, 100000);//冒泡排序 37.978000 s
	//shellSort( arr, 100000); //希尔排序 0.038000 s
	end = clock();
